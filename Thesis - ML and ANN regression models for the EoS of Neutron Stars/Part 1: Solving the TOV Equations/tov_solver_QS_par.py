...
# Defining a function that finds all the valid combinations of the Β_eff and Δ parameters
# for stable CFL quark matter, depending on the maximum selected values of B_eff and Δ respectively,
# the step in scanning the B_eff axis and Δ axis

def Beff_Delta_valid_combos(Beff_max,Beff_step,Delta_max,Delta_step):
    models_names = []
    valid_Beff = []
    valid_Delta = []
    valid_ms = []
...
def tov_sol_worker(task_id,EOS_name,Beff_value,Delta_value,ms_value,progress_queue):
    ...
    P_in_center_2: np.ndarray = np.arange(5,11,1) # higher pressure range
...
if __name__ == "__main__":
    ...
    valid_combos_results = Beff_Delta_valid_combos(Beff_max_val,Beff_step_val,Delta_max_val,Delta_step_val)
