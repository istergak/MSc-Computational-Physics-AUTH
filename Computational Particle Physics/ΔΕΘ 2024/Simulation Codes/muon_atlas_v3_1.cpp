...
// Version 3.1 (.cpp script to be compiled to .exe)
// Version characteristics:
// 1) Intro and outro page
// 2) Satistics for the drift time of the trajectories
...
// cl -nologo -MD -GR /EHsc /std:c++17 -Zc:__cplusplus muon_atlas_v3_1.cpp -I %ROOTSYS%\include /link -LIBPATH:%ROOTSYS%\lib 
...
// gain access to the native tools command prompt. Create an environment variable named ROOTSYS and store at it the path where the 
// ROOT app is stored (something like C:\root_v6.32.02). Also include the root directory to the System's path. 
