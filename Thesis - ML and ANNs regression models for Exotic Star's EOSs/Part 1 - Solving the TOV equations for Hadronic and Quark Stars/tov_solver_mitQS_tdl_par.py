...
if __name__ == "__main__":
    ...
    B_max_val = float(input("Give the maximum value of the bag parameter B in [MeV*fm^-3]:  "))
    while B_max_val<=57:
        B_max_val = float(input("The value must be greater than 57 [MeV*fm^-3]. Try again:  "))

    # Ask the user to give the step for the scan of the Beff area
    B_step_val = float(input("Give the step for the scan of the B parameter area in [MeV*fm^-3]:  "))
    while B_step_val<=0:
        B_step_val = float(input("The value must be positive. Try again:  "))
    print('-------------------------------------------------------------------------------')
