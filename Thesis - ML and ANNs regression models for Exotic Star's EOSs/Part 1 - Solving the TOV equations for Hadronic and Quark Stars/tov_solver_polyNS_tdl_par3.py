...
def EOS_polytropic(P,K_i,Γ_i,ρ_im1,P_im1,E_im1):
    if Γ_i!=1:
...
def EOS_polytropic_piecewise(P,ρ_bounds,Pi_vals,Ei_vals,Ki_vals,Γi_vals):
    points = len(Pi_vals)
    n = points-1 # number of pressure (or mass density) segments
    ...
    for i in range(0,n):
        if i==n-1:
            ...
            conditions.append((P>=Pi_vals[i])*(P<Pi_vals[i+1]))
        functions.append(EOS_polytropic(P,Ki_vals[i],Γi_vals[i],ρ_bounds[i],Pi_vals[i],Ei_vals[i]))

    return np.piecewise(P,conditions,functions)

# Defining the slope of the polytropic EOS for a single segment in high mass densities and high pressures
def EOS_slope_polytropic(P,K_i,Γ_i,ρ_im1,P_im1,E_im1):
    if Γ_i!=1:
...
def EOS_slope_polytropic_piecewise(P,ρ_bounds,Pi_vals,Ei_vals,Ki_vals,Γi_vals):
    points = len(Pi_vals)
    n = points -1 # number of pressure (or mass density) segments
    ...
    for i in range(0,n):
        conditions.append((P>Pi_vals[i])*(P<Pi_vals[i+1]))
        functions.append(EOS_slope_polytropic(P,Ki_vals[i],Γi_vals[i],ρ_bounds[i],Pi_vals[i],Ei_vals[i]))
...
# Defining the relation between pressure and baryon density at the linear region (barton density is expressed in fm^-3 units)
def n_b_transition(P,rho_frac_tr,P_tr,E_tr):
    return n_sat*rho_frac_tr*np.sqrt((2*P-P_tr+E_tr)/(E_tr+P_tr))            

# Defining an EOS function for the transition from polytropic behavior to linear behavior with slope 1
def EOS_transition(P,P_tr,E_tr):
    return E_tr + P - P_tr

# Defining the slope of the EOS function for the transition from polytropic behavior to linear behavior with slope 1
def EOS_transition_slope(P):
    return 1
...
def poly_param_calc(P_0,E_0,ρ_bounds,Γ_selections):
    ...
    for i in range(0,n):
        ...
        Ei = EOS_polytropic(Pi_vals[i+1],Ki,Γi,ρ_bounds[i],Pi_vals[i],Ei_vals[i])
        Ei_vals.append(Ei)
    
    # Locating the pressure at which the slope of the EOS becomes less than 1 (or
    # in equivalent the speed of sound c_s becomes greater than 1, violating causality)
    P_range = np.linspace(Pi_vals[0],Pi_vals[-1],10**4) # taking 10^4 points of pressure in the interval [P_0,P_n]
    P_final = Pi_vals[-1] # final pressure of the initial polytropic section
    P_tr = np.NaN
    E_tr = np.NaN
    rho_tr = np.NaN
    for i in range(0,len(P_range)):
        P = P_range[i]
        slope = EOS_slope_polytropic_piecewise(P,ρ_bounds,Pi_vals,Ei_vals,Ki_vals,Γ_selections)
        # Redefining the polytropic segments of pressure if slope less than 1 has been found
        if slope<1:
            P_tr = P
            E_tr = EOS_polytropic_piecewise(P_tr,ρ_bounds,Pi_vals,Ei_vals,Ki_vals,Γ_selections)
            rho_tr = ρ_poly_piecewise(P_tr,Pi_vals,Ki_vals,Γ_selections) 
            for i in range(0,len(Pi_vals)):
                P_bound = Pi_vals[i]
                if P_bound<P_tr:
                    continue
                else:
                    ...
                    break
            break
    results = [Pi_vals,Ei_vals,Ki_vals,P_final,rho_tr]
    return results
...
# for a given EOS
def tov_sol(p_0,ρ_sat_MeV,P_sat,r_step,total_EOS_name,main_EOS_func,main_EOS_func_sym,main_dEOS_func,ρ_bounds,Pi_vals,Ei_vals,Ki_vals,Γi_vals,rho_tr):
    # Defining the linear EOS as a function of pressure
    def transition_func(P):
        return EOS_transition(P,P_tr,E_tr)

    # Defining the polytropic EOS aa a function of pressure
    def polytropic_EOS(P):
        return EOS_polytropic_piecewise(P,ρ_bounds,Pi_vals,Ei_vals,Ki_vals,Γi_vals)
    ...
    n_b = None # baryon density at NS center
    ...
    P_tr = Pi_vals[-1]
    E_tr = Ei_vals[-1]
    rho_frac_tr = rho_tr/ρ_sat_MeV
    

    if Pc>=P_tr: # linear core area (c_s=1)
        E_c = EOS_transition(Pc,P_tr,E_tr)
        n_b = n_b_transition(Pc,rho_frac_tr,P_tr,E_tr)
        rho_frac = n_b/n_sat
        dE_dP = EOS_transition_slope(Pc)
    elif P_sat<=Pc<P_tr: # polytropic core area
        E_c = polytropic_EOS(Pc)
        rho_c = ρ_poly_piecewise(Pc,Pi_vals,Ki_vals,Γi_vals)
        rho_frac = rho_c/ρ_sat
        dE_dP = EOS_slope_polytropic_piecewise(Pc,ρ_bounds,Pi_vals,Ei_vals,Ki_vals,Γi_vals)  
    ...
    # Solving the TOV equations for the linear core section (c_s=1) of pressure
    EOS_func = transition_func
    dEOS_func = EOS_transition_slope
    while P_0>P_tr:
        ...
        # Appending values to the M_max, R_final, P_final and yR variables
    ...
    n = len(Pi_vals) # number of pressure segments bounds 
    k = n-1 # number of pressure segments
    for i in range(k,0,-1):
        def current_poly_EOS(P):
            return EOS_polytropic(P,Ki_vals[i-1],Γi_vals[i-1],ρ_bounds[i-1],Pi_vals[i-1],Ei_vals[i-1])
        def current_poly_EOS_slope(P):
            return EOS_slope_polytropic(P,Ki_vals[i-1],Γi_vals[i-1],ρ_bounds[i-1],Pi_vals[i-1],Ei_vals[i-1])
        EOS_func = current_poly_EOS
        dEOS_func = current_poly_EOS_slope
    ...
    filename = f'results_512_alt/{total_EOS_name}L_tdl_sol.csv'
    with open(filename,"a+") as file: 
        file.write(f"{P_out},{Pc},{E_c},{rho_frac},{dE_dP},{max(M_vals)},{R},{yR},{k2},{Lambda}\n")
...
def tov_sol_worker(task_id,Γ_selection_coded,Γ_choices,num_segments,main_EOS_name,main_EOS,main_EOS_sym,progress_queue):
    ...
    # Getting the changing pressure from the main EOS to the polytropic behavior, as well as the slope of the main EOS
    if main_EOS_name=="HLPS-2":
        P_sat = 1.722 # in MeV/fm^3
        main_dEOS = eos_lib_NS.dHLPS_2_num
    elif main_EOS_name=="HLPS-3":
        P_sat = 2.816 # in MeV/fm^3
        main_dEOS = eos_lib_NS.dHLPS_3_num    
    ...
    P_final = param_results[3]
    rho_tr = param_results[4]    
    ...
    if P_final<3000: 
        P_in_center_2: np.ndarray = np.arange(5,P_final,1) # higher pressure range
        P_in_center_total: np.ndarray = np.concatenate((P_in_center_1,P_in_center_2),axis=None)

    # maximum initial pressure more than 2500 MeV/fm^3 and Γ of 1st pressure segment greater than 2  
    else:      
    ...
    with open(filename,"w") as file:
        file.write("P_out,P_c,E_c,rho/rho_sat,dE_dP,M,R,yR,k2,L\n")
    ...
    for i in range(0,total_values):
        ...
        tov_sol(P_0,ρ_sat_MeV,P_sat,R_step,total_EOS_name,main_EOS,main_EOS_sym,main_dEOS,ρ_seg_bounds,Pi_vals,Ei_vals,Ki_vals,Γi_vals,rho_tr)
...
if __name__ == "__main__":
    ...
    main_EOS_offered_idx = ["0", "1"]  
    main_EOS_list.add_column("Offered main EOSs models",main_EOS_offered)
    main_EOS_list.add_column("Index",main_EOS_offered_idx)
    ...
    main_EOS_index = input(f"Type the main EOS index:  ")
    while main_EOS_index not in main_EOS_offered_idx:
        main_EOS_index = input(f"EOS not found. Please type a valid index for the main EoS:  ")
    if int(main_EOS_index)==0:
        main_EOS = main_EOS_formulas[main_EOS_names.index("HLPS-2")] # numerical formula of the selected main EOS
        main_EOS_sym = main_EOS_formulas_sym[main_EOS_names.index("HLPS-2")] # symbolic formula of the selected main EOS
        main_EOS_name = "HLPS-2"
    elif int(main_EOS_index)==1:
        main_EOS = main_EOS_formulas[main_EOS_names.index("HLPS-3")] # numerical formula of the selected main EOS
        main_EOS_sym = main_EOS_formulas_sym[main_EOS_names.index("HLPS-3")] # symbolic formula of the selected main EOS
        main_EOS_name = "HLPS-3"    
    print('-------------------------------------------------------------------------------')
