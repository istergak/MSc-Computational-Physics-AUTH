...
# Python Script: Py6a (Version 2 - Tidal Deformability Calculation Included)
# Name: tov_solver_cflQS_tdl_par.py
...
# for a given EOS
def tov_sol(p_0,r_step,Beff_val,Delta_val,EOS_name,EOS_formula,dEOS_formula):
...
def tov_sol_worker(task_id,EOS_name,Beff_value,Delta_value,ms_value,progress_queue):
    ...
    def EOS_function(P):
        return EOS_CFL_stable(P,Beff_value,Delta_value)
    
    # Getting the symbolic EOS function
    def EOS_function_sym(P):
        return EOS_CFL_stable_sym(P,Beff_value,Delta_value)
    
    # Calculating the first derivative of EOS
    ...
    with open(filename,"w") as file:
        file.write(f"P_out, P_c, E_c, dE_dP, M, R, yR_int, yR_ext, k2, L, B_eff={Beff_value:.1f},Delta={Delta_value:.1f}\n")
    ...
    for i in range(0,total_values):
        ...
        tov_sol(P_0,R_step,Beff_value,Delta_value,EOS_name,EOS_function,dEOS_function)
